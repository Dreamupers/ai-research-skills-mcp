# ai research mcps and skills

MCP (Model Context Protocol) 服务集合，为 agent 提供 arXiv 论文搜索、飞书文档上传等能力；`skills/` 下为可复用的 Agent 技能说明（供 Cursor 等挂载使用）。
...
将本地 Markdown 文件上传为飞书云文档，支持公式、图片、表格等富文本转换。
  ...
  "mcpServers": {
    ...
    "feishu-doc-upload": {
      ...
      "env": {
        ...
        "FEISHU_DEFAULT_FOLDER_TOKEN": "your_default_folder_token"
      }
    }
  }
}
...
与本仓库 MCP 服务配套的 Markdown 技能，可复制到个人技能目录或按需引用：


| 目录                         | 技能名                  | 说明                                                                          |
| -------------------------- | -------------------- | --------------------------------------------------------------------------- |
| `skills/arxiv-paper-read/` | `arxiv-paper-reader` | 按标题/arXiv id 读论文、写中文笔记（输出目录与命名见该技能内说明）                                      |
| `skills/cc-connect-send/`  | `cc-connect-send`    | 通过本机 `cc-connect send` 向飞书/Telegram 等会话发文本、文件、图片（需已安装并配置 cc-connect，且有活跃会话） |
...
├── skills/
│   ├── arxiv-paper-read/     # Agent 技能：读 arXiv 论文写笔记
│   └── cc-connect-send/      # Agent 技能：cc-connect 发消息/附件
├── pyproject.toml
├── uv.lock
└── README.md
```
